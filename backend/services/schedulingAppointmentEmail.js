...
function getStoreNotificationEmail() {
    const explicit = String(process.env.Scheduling_NOTIFY_EMAIL || process.env.STORE_NOTIFY_EMAIL || '').trim();
...
function urls(bookingId, email) {
    ...
    return {
        confirmation: `${base}/scheduling-confirmation.html?${q}`,
        manage: `${base}/scheduling-manage-appointment.html?${q}`
    };
}
...
async function sendBookingReceivedEmail(booking) {
    ...
    const subject = `${process.env.STORE_NAME || 'Business One'} — Scheduling appointment request #${booking.bookingId}`;
    const html = `
        <div style="font-family:Inter,system-ui,sans-serif;color:#111827;max-width:560px;">
            <h2 style="color:#ff9b1f;margin:0 0 8px;">Your Scheduling appointment request is received</h2>
    ...
    const text = [
        `Your Scheduling appointment request is received.`,
    ...
    logger.info('Scheduling booking email links', { confirmation: links.confirmation, manage: links.manage });
    await sendEmail({ to: email, subject, html, text, logTag: 'Scheduling booking email' });
}
...
async function sendBookingReceivedStoreEmail(booking) {
    ...
    const subject = `[Scheduling] New booking #${booking.bookingId} — ${customerName}`;
    const html = `
        <div style="font-family:Inter,system-ui,sans-serif;color:#111827;max-width:560px;">
            <h2 style="color:#ff9b1f;margin:0 0 8px;">New Scheduling appointment booked online</h2>
    ...
    await sendEmail({ to: storeEmail, subject, html, text: subject, logTag: 'Scheduling new-booking store notification' });
}
...
function storeAdminLinkHtml(bookingId) {
    ...
    const adminLink = `${base}${query}#scheduling`;
    return `<p><a href="${escapeHtml(adminLink)}" style="background:#ff9b1f;color:#fff;padding:10px 20px;text-decoration:none;border-radius:5px;display:inline-block;">Open admin — Scheduling bookings</a></p>`;
}
...
async function sendAppointmentCancelledEmail(booking) {
    ...
    const name = `${booking.firstName || ''}`.trim() || 'there';
    const when = `${formatDate(booking.preferredDate)} at ${formatTime(booking.preferredTime)}`;
    const subject = `${process.env.STORE_NAME || 'Business One'} — Scheduling appointment cancelled (#${booking.bookingId})`;
    const html = `
        <div style="font-family:Inter,system-ui,sans-serif;color:#111827;max-width:560px;">
    ...
    const text = [`Your Scheduling appointment #${booking.bookingId} (${when}) is cancelled.`, `Book again: ${getStorefrontPublicBaseUrl()}`].join('\n');
    await sendEmail({ to: email, subject, html, text, logTag: 'Scheduling cancellation email' });
}
...
async function sendAppointmentCancelledStoreEmail(booking) {
    ...
    const customerName = [booking.firstName, booking.lastName].filter(Boolean).join(' ') || 'Customer';
    const when = `${formatDate(booking.preferredDate)} at ${formatTime(booking.preferredTime)}`;
    const subject = `[Scheduling] Cancelled #${booking.bookingId} — ${customerName}`;
    const html = `
        <div style="font-family:Inter,system-ui,sans-serif;color:#111827;max-width:560px;">
            <h2 style="color:#ff9b1f;margin:0 0 8px;">Scheduling appointment cancelled online</h2>
            <p><strong>#${escapeHtml(booking.bookingId)}</strong> — ${escapeHtml(customerName)}<br>
               ${escapeHtml(when)}<br>
               ${escapeHtml(booking.email)} · ${escapeHtml(booking.phone || '—')}</p>
            ${storeAdminLinkHtml(booking.bookingId)}
        </div>`;
    await sendEmail({ to: storeEmail, subject, html, text: subject, logTag: 'Scheduling cancellation store notification' });
}
...
async function sendAppointmentRescheduledEmail(booking, previousDate, previousTime) {
    ...
    const subject = `${process.env.STORE_NAME || 'Business One'} — Scheduling appointment rescheduled (#${booking.bookingId})`;
    const html = `
        <div style="font-family:Inter,system-ui,sans-serif;color:#111827;max-width:560px;">
    ...
    await sendEmail({ to: email, subject, html, text, logTag: 'Scheduling reschedule email' });
}
...
async function sendAppointmentRescheduledStoreEmail(booking, previousDate, previousTime) {
    ...
    const newWhen = `${formatDate(booking.preferredDate)} at ${formatTime(booking.preferredTime)}`;
    const oldWhen = `${formatDate(previousDate)} at ${formatTime(previousTime)}`;
    const subject = `[Scheduling] Rescheduled #${booking.bookingId} — ${customerName}`;
    const html = `
        <div style="font-family:Inter,system-ui,sans-serif;color:#111827;max-width:560px;">
            <h2 style="color:#ff9b1f;margin:0 0 8px;">Scheduling appointment rescheduled online</h2>
            <p><strong>#${escapeHtml(booking.bookingId)}</strong> — ${escapeHtml(customerName)}<br>
    ...
    await sendEmail({ to: storeEmail, subject, html, text: subject, logTag: 'Scheduling reschedule store notification' });
}
...
async function sendChangeRequestReceivedEmail(booking, requestType) {
    ...
    const text = [
        ...
        .filter(Boolean)
        .join('\n');
    await sendEmail({ to: email, subject, html, text, logTag: 'Scheduling change-request email' });
}

/** Notify store staff when a customer requests cancel/reschedule. */
async function sendChangeRequestStoreNotificationEmail(booking, requestType) {
    const storeEmail = getStoreNotificationEmail();
    if (!storeEmail) {
        logger.warn('Scheduling store notification skipped — no Scheduling_NOTIFY_EMAIL or SMTP_USER');
        return;
    }
    ...
    const adminLinkPlain = `${getAdminAppUrl()}?booking=${booking.bookingId}#scheduling`;

    const subject = `[Scheduling] ${label} request #${booking.bookingId} — ${customerName}`;
    const html = `
        <div style="font-family:Inter,system-ui,sans-serif;color:#111827;max-width:560px;">
            <h2 style="color:#ff9b1f;margin:0 0 8px;">Scheduling ${escapeHtml(label)} request</h2>
            <p><strong>Confirmation #:</strong> ${escapeHtml(booking.bookingId)}<br>
            ...
            ${extra}
            ${adminLinkHtml || '<p>Review this request in the admin panel under Scheduling bookings.</p>'}
        </div>`;
    const text = [
        `Scheduling ${label} request #${booking.bookingId}`,
    ...
    await sendEmail({
        ...
        html,
        text,
        logTag: 'Scheduling change-request store notification'
    });
}
...
async function sendAdminResolutionEmail(booking) {
    ...
    const html = `
        <div style="font-family:Inter,system-ui,sans-serif;color:#111827;max-width:560px;">
            <h2 style="color:#ff9b1f;margin:0 0 8px;">Your Scheduling appointment was ${escapeHtml(statusLabel)}</h2>
            <p><strong>Confirmation #:</strong> ${escapeHtml(booking.bookingId)}<br>
    ...
    const text = [
        `Your Scheduling appointment was ${statusLabel}.`,
        `Confirmation #: ${booking.bookingId}`,
        `Appointment: ${when}`,
        `Details: ${links.confirmation}`
    ].join('\n');
    await sendEmail({ to: email, subject, html, text, logTag: 'Scheduling admin-update email' });
}
...
async function sendStaffCancelledCustomerEmail(booking) {
    ...
    const when = `${formatDate(booking.preferredDate)} at ${formatTime(booking.preferredTime)}`;
    const links = urls(booking.bookingId, email);
    const subject = `${process.env.STORE_NAME || 'Business One'} — Your Scheduling appointment was cancelled (#${booking.bookingId})`;
    const html = `
        <div style="font-family:Inter,system-ui,sans-serif;color:#111827;max-width:560px;">
            <h2 style="color:#ff9b1f;margin:0 0 8px;">Your appointment was cancelled</h2>
            <p>Hello ${escapeHtml(name)},</p>
            <p>Business One has cancelled your Scheduling session scheduled for <strong>${escapeHtml(when)}</strong> (confirmation #${escapeHtml(booking.bookingId)}).</p>
            <p>Questions or want to rebook? Call us at (850) 290-2084 or visit our website.</p>
            <p><a href="${escapeHtml(`${getStorefrontPublicBaseUrl()}/index.html`)}" style="background:#ff9b1f;color:#fff;padding:10px 20px;text-decoration:none;border-radius:5px;display:inline-block;">Visit Business One</a></p>
        </div>`;
    const text = [`Your Scheduling appointment #${booking.bookingId} (${when}) was cancelled by our team.`, links.manage].join('\n');
    await sendEmail({ to: email, subject, html, text, logTag: 'Scheduling staff-cancel customer email' });
}
...
async function sendStaffRescheduledCustomerEmail(booking, previousDate, previousTime) {
    ...
    const oldWhen = `${formatDate(previousDate)} at ${formatTime(previousTime)}`;
    const links = urls(booking.bookingId, email);
    const subject = `${process.env.STORE_NAME || 'Business One'} — Your Scheduling appointment was updated (#${booking.bookingId})`;
    const html = `
        <div style="font-family:Inter,system-ui,sans-serif;color:#111827;max-width:560px;">
            <h2 style="color:#ff9b1f;margin:0 0 8px;">Your appointment time was updated</h2>
            <p>Hello ${escapeHtml(name)},</p>
            <p>Our team updated your Scheduling appointment (confirmation #${escapeHtml(booking.bookingId)}):</p>
            <p><strong>Previous:</strong> ${escapeHtml(oldWhen)}<br>
               <strong>New:</strong> ${escapeHtml(newWhen)}</p>
            <p><a href="${escapeHtml(links.confirmation)}" style="background:#ff9b1f;color:#fff;padding:10px 20px;text-decoration:none;border-radius:5px;display:inline-block;margin-right:8px;">View confirmation</a>
               <a href="${escapeHtml(links.manage)}" style="color:#ff9b1f;">Manage appointment</a></p>
    ...
    await sendEmail({ to: email, subject, html, text, logTag: 'Scheduling staff-reschedule customer email' });
}
