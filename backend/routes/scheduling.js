// Scheduling (Electro Dermal Stress Analysis) Service Routes
...
const {
    ...
    isDateBlocked,
    slotsForBlockedDay
} = require('../utils/schedulingAvailability');
const { listBlockedDates, blockedDateSet } = require('../services/schedulingBlockedDates');
...
function isSchedulingPaymentConfigured() {
...
async function getSchedulingServicePrice(pool) {
    const [settings] = await pool.execute(
        `SELECT value FROM settings WHERE key_name = 'scheduling_service_price' LIMIT 1`
    );
...
async function loadBookingForCustomer(pool, bookingId, email) {
    ...
    const [rows] = await pool.execute(
        `SELECT id, first_name, last_name, email, phone,
           ...
           FROM scheduling_bookings WHERE id = ? LIMIT 1`,
        [id]
    );
...
async function isSlotAvailable(pool, dateStr, timeHm, excludeBookingId = null) {
    ...
    if (googleCalendar.isAvailable()) {
        try {
            ...
            logger.warn('Scheduling slot check: Google Calendar unavailable, using database only', {
                error: err.message
            });
        }
    }
...
async function syncBookingCalendarEvent(pool, row) {
    if (!isSchedulingBookingFinalized(row)) {
        return;
    }
    ...
    try {
        ...
        if (created?.eventId) {
            await pool.execute(
                'UPDATE scheduling_bookings SET google_calendar_event_id = ? WHERE id = ?',
...
function isSchedulingBookingFinalized(row) {
    if (!row) return false;
...
async function finalizeSchedulingBookingAfterInsert(pool, bookingId, payload) {
    const {
        firstName,
        lastName,
        email,
    ...
    try {
        await withTimeout(
            sendSchedulingBookingConfirmations(bookingId, {
                firstName,
                lastName,
                email,
                phone,
        ...
        logger.error('Scheduling booking notification email error (booking saved):', emailErr);
    }
}

async function sendSchedulingBookingConfirmations(bookingId, emailFields) {
    const emailPayload = {
        bookingId,
...
// Get business hours for Scheduling service
router.get('/hours', async (req, res) => {
    try {
        ...
        logger.error('Scheduling hours fetch error:', error);
...
router.get('/info', async (req, res) => {
    try {
        const [settings] = await req.pool.execute(`
            ...
            FROM settings 
            WHERE key_name IN ('scheduling_service_enabled', 'scheduling_service_price', 'scheduling_service_description')
        `);
        ...
        res.json({
            enabled: serviceInfo.scheduling_service_enabled === 'true',
            price: parseFloat(serviceInfo.scheduling_service_price || 75.00),
            description: serviceInfo.scheduling_service_description || 'Electro Dermal Stress Analysis - A non-invasive health assessment technique'
        });
    } catch (error) {
        logger.error('Scheduling info fetch error:', error);
...
router.get('/booking-context', async (req, res) => {
    try {
        ...
        const price = await getSchedulingServicePrice(req.pool);
        const paymentConfigured = isSchedulingPaymentConfigured();
        ...
        if (authUser) {
            try {
                ...
                logger.warn('Scheduling booking-context saved cards:', cardErr.message);
            }
        }
        ...
        logger.error('Scheduling booking context error:', error);
        res.status(500).json({ error: 'Internal server error' });
    }
});
...
// Book Scheduling appointment
router.post('/book', schedulingBookingValidation, async (req, res) => {
    try {
        const {
            firstName,
            lastName,
        ...
        const paymentRequired = isSchedulingPaymentConfigured();
        ...
        if (paymentRequired) {
            ...
            if (!payment_token && !hasSavedCard) {
                return res.status(400).json({
                    error: 'Payment is required to book your Scheduling session.',
        ...
        const servicePrice = await getSchedulingServicePrice(req.pool);
        ...
        if (paymentRequired) {
            ...
            if (!(await isSlotAvailable(req.pool, preferredDate, normalizedTime))) {
                if (paymentReference) {
                    ...
                    if (!voidResult.ok) {
                        logger.error('Scheduling void failed after slot conflict', {
        ...
        res.status(201).json({
            message: 'Scheduling appointment booking submitted successfully',
        ...
        if (shouldFinalize) {
            void finalizeSchedulingBookingAfterInsert(req.pool, bookingId, {
                firstName,
                lastName,
                email,
                phone,
        ...
        logger.error('Scheduling booking error:', error);
        res.status(500).json({ error: 'Internal server error' });
    }
});

// Get user's Scheduling bookings (requires authentication)
router.get('/bookings', async (req, res) => {
    try {
        ...
        logger.error('Scheduling bookings fetch error:', error);
        res.status(500).json({ error: 'Internal server error' });
    }
});
...
router.get('/bookings/:id/confirmation-summary', async (req, res) => {
    try {
        ...
        logger.error('Scheduling confirmation summary error:', error);
...
router.get('/bookings/:id/manage', async (req, res) => {
    try {
        ...
        logger.error('Scheduling manage booking error:', error);
        res.status(500).json({ error: 'Failed to load appointment' });
    }
});

// Customer self-service cancel (immediate)
router.post('/bookings/:id/cancel-appointment', schedulingCustomerEmailValidation, async (req, res) => {
    try {
        const bookingId = Number(req.params.id);
        ...
        try {
            ...
            logger.error('Scheduling cancellation email error (cancel saved):', emailErr);
        }
        ...
        logger.error('Scheduling customer cancel error:', error);
        res.status(500).json({ error: 'Failed to cancel appointment' });
    }
});

// Customer self-service reschedule (immediate when slot is available)
router.post('/bookings/:id/reschedule-appointment', schedulingCustomerRescheduleValidation, async (req, res) => {
    try {
        const bookingId = Number(req.params.id);
        ...
        try {
            ...
            logger.error('Scheduling reschedule email error (reschedule saved):', emailErr);
        }
        ...
        logger.error('Scheduling customer reschedule error:', error);
...
router.get('/bookings/:id', async (req, res) => {
    try {
        ...
        logger.error('Scheduling booking fetch error:', error);
        res.status(500).json({ error: 'Internal server error' });
    }
});
...
router.put('/bookings/:id/cancel', async (req, res) => {
    try {
        ...
        let query = 'UPDATE scheduling_bookings SET status = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ? AND status IN (?, ?)';
        ...
        try {
            const [booking] = await req.pool.execute(
                'SELECT google_calendar_event_id FROM scheduling_bookings WHERE id = ?',
                [id]
            );
        ...
        logger.error('Scheduling booking cancellation error:', error);
        res.status(500).json({ error: 'Internal server error' });
    }
});
...
router.post('/create-calendar-event', async (req, res) => {
    try {
        ...
        if (!isSchedulingBookingFinalized(rows[0])) {
            return res.status(402).json({
