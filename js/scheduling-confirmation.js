// Scheduling appointment thank-you page
...
function manageAppointmentUrl(data) {
    ...
    return `scheduling-manage-appointment.html?${params.toString()}`;
}
...
function renderConfirmation(root, data) {
    ...
    root.innerHTML = `
        ...
        <h1>Your Scheduling appointment is booked</h1>
    ...
    if (email) {
        try {
            const res = await fetch(
                `${schedulingApiBase()}/bookings/${encodeURIComponent(bookingId)}/confirmation-summary?email=${encodeURIComponent(email)}`,
            ...
            console.warn('Could not load Scheduling booking summary:', e);
        }
    }
