...
function ensureCallStoreModal() {
    ...
    modal.innerHTML = `
        <div class="scheduling-modal-overlay" data-call-store-close></div>
        <div class="scheduling-modal-content call-store-modal-content">
            <div class="scheduling-modal-header">
                <h2 id="call-store-title">Call the Store</h2>
                <button type="button" class="scheduling-modal-close" data-call-store-close aria-label="Close">
                    ${CLOSE_ICON_SVG}
                </button>
            </div>
            <div class="scheduling-modal-body call-store-modal-body">
...
function renderLookupForm(root) {
    root.innerHTML = `
        <h1>Manage your Scheduling appointment</h1>
        ...
        <form id="manage-lookup-form" class="manage-form" novalidate>
            <div id="manage-form-message" class="scheduling-form-message" role="alert" hidden></div>
    ...
    document.getElementById('manage-lookup-form').addEventListener('submit', (e) => {
        ...
        window.location.href = `scheduling-manage-appointment.html?booking=${encodeURIComponent(bookingId)}&email=${encodeURIComponent(email)}`;
    });
}
...
class ManageAppointmentUI {
    ...
    async loadSlotsForDate(dateYmd) {
        const url = `${schedulingApiBase()}/available-slots?date=${encodeURIComponent(dateYmd)}&excludeBookingId=${encodeURIComponent(this.data.bookingId)}&_=${Date.now()}`;
        try {
        ...
        this.root.innerHTML = `
            ...
            <div id="cancel-panel" hidden>
                <p class="manage-lead" style="text-align:left;color:#991b1b;">
                    This will cancel your Scheduling session and remove it from our calendar.
                </p>
                <div class="manage-actions">
            ...
            <div class="manage-actions" style="margin-top:1.5rem;">
                <button type="button" class="btn btn-secondary" data-call-store>Call the Store</button>
                <a href="scheduling-confirmation.html?booking=${encodeURIComponent(this.data.bookingId)}&email=${encodeURIComponent(this.data.email)}" class="btn btn-outline-secondary">View confirmation</a>
                <a href="index.html" class="btn btn-outline-secondary">Back to home</a>
            </div>`
                    : `<div class="manage-actions">
    ...
    async submitReschedule() {
        ...
        try {
            const res = await fetch(
                `${schedulingApiBase()}/bookings/${encodeURIComponent(this.data.bookingId)}/reschedule-appointment`,
                {
                    method: 'POST',
            ...
            if (res.ok) {
                ...
                setTimeout(() => {
                    window.location.href = `scheduling-confirmation.html?${params.toString()}`;
                }, 1200);
                return;
            }
    ...
    async submitCancel() {
        const confirmed =
            typeof window.showEdsConfirm === 'function'
                ? await window.showEdsConfirm({
                      ...
                      message:
                          'This will cancel your Scheduling session and remove it from our calendar. You can book a new time anytime.',
        ...
        try {
            const res = await fetch(
                `${schedulingApiBase()}/bookings/${encodeURIComponent(this.data.bookingId)}/cancel-appointment`,
                {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json', Accept: 'application/json' },
...
async function initManageAppointment() {
    ...
    try {
        const res = await fetch(
            `${schedulingApiBase()}/bookings/${encodeURIComponent(bookingId)}/manage?email=${encodeURIComponent(email)}`,
