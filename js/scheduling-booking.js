// Scheduling Booking System — calendar modal, slot availability, confirmation redirect

const Scheduling_NATIVE_FETCH = (() => {
    const native = window.__nativeFetch;
    return native ? native.bind(window) : fetch.bind(window);
})();

class SchedulingBookingSystem {
    ...
    getApiBaseUrl() {
        ...
        return origin ? `${origin}/api/scheduling` : '/api/scheduling';
    }
    ...
    setupModal() {
        const existing = document.getElementById('scheduling-booking-modal');
        const version = existing?.getAttribute('data-scheduling-modal-version');
        const isCurrentModal =
            existing &&
            version === String(SchedulingBookingSystem.MODAL_VERSION) &&
            existing.querySelector('#scheduling-step-schedule');

        if (isCurrentModal) {
            SchedulingBookingSystem.modalReady = true;
            return;
        }
        ...
        const prevIcon =
            '<svg class="scheduling-calendar-nav-icon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true" focusable="false"><path d="M15.41 7.41L14 6l-6 6 6 6 1.41-1.41L10.83 12z"/></svg>';
        const nextIcon =
            '<svg class="scheduling-calendar-nav-icon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true" focusable="false"><path d="M10 6L8.59 7.41 13.17 12l-4.58 4.59L10 18l6-6z"/></svg>';

        const modalHTML = `
            <div id="scheduling-booking-modal" class="scheduling-modal" data-scheduling-modal-version="${SchedulingBookingSystem.MODAL_VERSION}" aria-hidden="true" role="dialog" aria-labelledby="scheduling-modal-title">
                <div class="scheduling-modal-overlay"></div>
                <div class="scheduling-modal-content">
                    <div class="scheduling-modal-header">
                        <h2 id="scheduling-modal-title">Book Your Scheduling Session</h2>
                        <button type="button" class="scheduling-modal-close" aria-label="Close booking modal">
                    ...
                    <div class="scheduling-modal-body">
                        <div id="scheduling-form-message" class="scheduling-form-message" role="alert" hidden></div>
                        <div class="scheduling-step-indicator" id="scheduling-step-indicator" aria-hidden="true">
                            <span class="scheduling-step-dot active" data-step="schedule">1</span>
                            <span class="scheduling-step-line"></span>
                            <span class="scheduling-step-dot" data-step="details">2</span>
                            <span class="scheduling-step-line" id="scheduling-step-payment-line"></span>
                            <span class="scheduling-step-dot" data-step="payment" id="scheduling-step-payment-dot">3</span>
                        </div>
                        <div class="scheduling-booking-container">
                            <div id="scheduling-step-schedule" class="scheduling-step-panel">
                                <div class="scheduling-calendar-section">
                                    <div class="scheduling-calendar-header">
                                        <button type="button" class="scheduling-calendar-nav" id="prev-month" aria-label="Previous month">${prevIcon}</button>
                                        <h3 id="calendar-month-year"></h3>
                                        <button type="button" class="scheduling-calendar-nav" id="next-month" aria-label="Next month">${nextIcon}</button>
                                    </div>
                                    <div class="scheduling-calendar-grid" id="calendar-grid"></div>
                                    <div class="scheduling-time-slots" id="time-slots"></div>
                                </div>
                                <div class="scheduling-step-actions">
                                    <button type="button" class="btn btn-secondary" id="scheduling-cancel-btn">Cancel</button>
                                    <button type="button" class="btn btn-primary" id="scheduling-schedule-continue" disabled>Continue</button>
                                </div>
                            </div>
                            <div id="scheduling-step-details" class="scheduling-step-panel" hidden>
                                <p class="scheduling-selected-summary" id="scheduling-selected-summary"></p>
                                <form id="scheduling-booking-form" novalidate>
                                    <div class="form-group">
                                        <label for="scheduling-first-name">First Name *</label>
                                        <input type="text" id="scheduling-first-name" name="firstName" required autocomplete="given-name">
                                    </div>
                                    <div class="form-group">
                                        <label for="scheduling-last-name">Last Name *</label>
                                        <input type="text" id="scheduling-last-name" name="lastName" required autocomplete="family-name">
                                    </div>
                                    <div class="form-group">
                                        <label for="scheduling-email">Email *</label>
                                        <input type="email" id="scheduling-email" name="email" required autocomplete="email">
                                    </div>
                                    <div class="form-group">
                                        <label for="scheduling-phone">Phone *</label>
                                        <input type="tel" id="scheduling-phone" name="phone" required autocomplete="tel"
                                            placeholder="(555) 555-0100" maxlength="14" inputmode="numeric">
                                    </div>
                                    <div class="form-group">
                                        <label for="scheduling-notes">Additional Notes</label>
                                        <textarea id="scheduling-notes" name="notes" rows="2" autocomplete="off"></textarea>
                                    </div>
                                    <div class="scheduling-step-actions">
                                        <button type="button" class="btn btn-secondary" id="scheduling-details-back">Back</button>
                                        <button type="submit" class="btn btn-primary" id="scheduling-details-continue">Continue</button>
                                    </div>
                                </form>
                            </div>
                            <div id="scheduling-step-payment" class="scheduling-step-panel" hidden>
                                <p class="scheduling-selected-summary" id="scheduling-payment-summary"></p>
                                <div class="scheduling-payment-section" id="scheduling-payment-section">
                                    <div class="scheduling-payment-summary">
                                        <span class="scheduling-payment-label">Session fee</span>
                                        <span class="scheduling-payment-amount" id="scheduling-payment-amount">$75.00</span>
                                    </div>
                                    <p class="scheduling-payment-note">You chose your appointment time on step 1. Your card is charged here — the calendar invite is sent only after payment succeeds.</p>
                                    <p id="scheduling-payment-signin-hint" class="scheduling-payment-signin-hint" hidden>
                                        Have an account with a saved card?
                                        <button type="button" class="scheduling-link-btn" id="scheduling-payment-signin-btn">Sign in</button>
                                        to use it instead of re-entering your card.
                                    </p>
                                    <div id="scheduling-saved-cards-block" class="scheduling-saved-cards-block" hidden>
                                        <label for="scheduling-saved-card-select" class="form-label">Payment method</label>
                                        <select id="scheduling-saved-card-select" class="form-select">
                                            <option value="">Use a new card</option>
                                        </select>
                                    </div>
                                    <div id="scheduling-nmi-collect-fields" class="scheduling-nmi-collect" style="display: none;">
                                        <div class="form-group">
                                            <span id="scheduling-ccnumber-label" class="form-label">Card number *</span>
                                            <div id="scheduling-ccnumber" class="nmi-field-host" role="group" aria-labelledby="scheduling-ccnumber-label"></div>
                                        </div>
                                        <div class="form-row">
                                            <div class="form-group">
                                                <span id="scheduling-ccexp-label" class="form-label">Expiration *</span>
                                                <div id="scheduling-ccexp" class="nmi-field-host" role="group" aria-labelledby="scheduling-ccexp-label"></div>
                                            </div>
                                            <div class="form-group">
                                                <span id="scheduling-cvv-label" class="form-label">CVV *</span>
                                                <div id="scheduling-cvv" class="nmi-field-host" role="group" aria-labelledby="scheduling-cvv-label"></div>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                <div class="scheduling-step-actions">
                                    <button type="button" class="btn btn-secondary" id="scheduling-payment-back">Back</button>
                                    <button type="button" class="btn btn-primary" id="scheduling-pay-btn">Pay &amp; Book Appointment</button>
                                </div>
                            </div>
                        </div>
                    </div>
        ...
        if (window.HMHERBS_PHONE_US) {
            const modal = document.getElementById('scheduling-booking-modal');
    ...
    mountBookingModal() {
        ...
        modal.scrollTop = 0;
        const panel = modal.querySelector('.scheduling-modal-content');
        const body = modal.querySelector('.scheduling-modal-body');
    ...
    fixFormAttributes() {
        const form = document.getElementById('scheduling-booking-form');
        if (!form) return;
    ...
    ensureEventListeners() {
        ...
        const closeBtn = modal.querySelector('.scheduling-modal-close');
        const overlay = modal.querySelector('.scheduling-modal-overlay');
        const cancelBtn = document.getElementById('scheduling-cancel-btn');
        const form = document.getElementById('scheduling-booking-form');
        const prevMonth = document.getElementById('prev-month');
        const nextMonth = document.getElementById('next-month');
        const bookBtn = document.getElementById('scheduling-book-btn');
        const scheduleContinue = document.getElementById('scheduling-schedule-continue');
        const detailsBack = document.getElementById('scheduling-details-back');
        const paymentBack = document.getElementById('scheduling-payment-back');
        const payBtn = document.getElementById('scheduling-pay-btn');
        const savedCardSelect = document.getElementById('scheduling-saved-card-select');
        ...
        if (savedCardSelect) {
            savedCardSelect.addEventListener('change', async () => {
                ...
                const collect = document.getElementById('scheduling-nmi-collect-fields');
                if (collect) {
        ...
        const signInBtn = document.getElementById('scheduling-payment-signin-btn');
        if (signInBtn) {
    ...
    lockPageScroll() {
        ...
        document.documentElement.classList.add('scheduling-modal-open');
        document.body.classList.add('scheduling-modal-open');
    ...
    unlockPageScroll() {
        ...
        document.documentElement.classList.remove('scheduling-modal-open');
        document.body.classList.remove('scheduling-modal-open');
        window.scrollTo(0, restoreY);
    }

    showFormMessage(message, type = 'error') {
        const box = document.getElementById('scheduling-form-message');
        if (box) {
            box.hidden = false;
            box.className = `scheduling-form-message scheduling-form-message-${type}`;
    ...
    async refreshBookingContextForSubmit() {
        ...
        try {
            ...
            console.error('Scheduling booking context refresh failed:', error);
    ...
    async loadBookingContext(year, month) {
        ...
        try {
            ...
            const amountEl = document.getElementById('scheduling-payment-amount');
            if (amountEl) amountEl.textContent = `$${this.servicePrice.toFixed(2)}`;

            const payDot = document.getElementById('scheduling-step-payment-dot');
            const payLine = document.getElementById('scheduling-step-payment-line');
    ...
    updatePaymentSignInHint() {
        const hint = document.getElementById('scheduling-payment-signin-hint');
        if (!hint) return;
    ...
    renderSavedCardsSelect() {
        const block = document.getElementById('scheduling-saved-cards-block');
        const select = document.getElementById('scheduling-saved-card-select');
    ...
    updateSelectedSummary() {
        ...
        const detailsEl = document.getElementById('scheduling-selected-summary');
        const paymentEl = document.getElementById('scheduling-payment-summary');
    ...
    showStep(step) {
        this._step = step;
        const schedule = document.getElementById('scheduling-step-schedule');
        const details = document.getElementById('scheduling-step-details');
        const payment = document.getElementById('scheduling-step-payment');
        if (schedule) schedule.hidden = step !== 'schedule';
        if (details) details.hidden = step !== 'details';
        if (payment) payment.hidden = step !== 'payment';

        document.querySelectorAll('#scheduling-step-indicator .scheduling-step-dot').forEach((dot) => {
        ...
        const title = document.getElementById('scheduling-modal-title');
        if (title) {
            if (step === 'schedule') title.textContent = 'Choose your Scheduling date & time';
    ...
    updateScheduleContinueButton() {
        const btn = document.getElementById('scheduling-schedule-continue');
    ...
    prefillFromLoggedInUser() {
        ...
        setVal('scheduling-first-name', user.firstName || user.first_name);
        setVal('scheduling-last-name', user.lastName || user.last_name);
        setVal('scheduling-email', user.email);
        setVal('scheduling-phone', user.phone);
    }
    ...
    goToDetailsStep() {
        ...
        const detailsBtn = document.getElementById('scheduling-details-continue');
        if (detailsBtn) {
    ...
    async goToPaymentStep(bookingData) {
        ...
        try {
            ...
            console.error('Scheduling payment step error:', err);
            this.showFormMessage(
    ...
    buildNmiCollectConfigureOptions() {
        ...
        return {
            ...
            fields: {
                ccnumber: { selector: '#scheduling-ccnumber', placeholder: 'Card number' },
                ccexp: { selector: '#scheduling-ccexp', placeholder: 'MM / YY' },
                cvv: { selector: '#scheduling-cvv', placeholder: 'CVV' }
            },
            country: 'US',
            currency: 'USD',
    ...
    async initNmiPayment(cfg) {
        ...
        if (document.getElementById('scheduling-nmi-collect-script')) {
            if (typeof window.CollectJS !== 'undefined') {
                this.nmiEnabled = true;
                this.nmiScriptReady = true;
                const block = document.getElementById('scheduling-nmi-collect-fields');
        ...
        script.id = 'scheduling-nmi-collect-script';
        script.src = url;
        script.async = true;
    ...
    buildConfirmationUrl({ bookingId, email, preferredDate, preferredTime, firstName, lastName }) {
        ...
        return `${base}scheduling-confirmation.html?${params.toString()}`;
    }
    ...
    redirectToConfirmation(details) {
        ...
        setTimeout(() => {
            if (!/\/scheduling-confirmation\.html/i.test(window.location.pathname)) {
    ...
    async submitBooking(bookingData) {
        ...
        try {
            const response = await Scheduling_NATIVE_FETCH(`${this.apiBaseUrl}/book`, {
                method: 'POST',
    ...
    async handleFormSubmit(e) {
        ...
        const form =
            (e.target && e.target.id === 'scheduling-booking-form' ? e.target : null) ||
            (e.currentTarget && e.currentTarget.id === 'scheduling-booking-form' ? e.currentTarget : null) ||
            document.getElementById('scheduling-booking-form');
        if (!form) return;
        ...
        try {
            ...
            console.error('Scheduling form submit error:', error);
...
let schedulingBookingSystem;

function openSchedulingBooking(e) {
    if (e && typeof e.preventDefault === 'function') {
        e.preventDefault();
    }
    if (!schedulingBookingSystem) {
        schedulingBookingSystem = new SchedulingBookingSystem();
    }
    schedulingBookingSystem.setupModal();
    schedulingBookingSystem.openModal();
}

function handleEdsBookingFormSubmit(e) {
    const form = e.target;
    if (!form || form.id !== 'scheduling-booking-form') return;
    ...
    schedulingBookingSystem.handleFormSubmit(e);
}
...
if (!window.__schedulingBookingSubmitCaptureBound) {
    window.__schedulingBookingSubmitCaptureBound = true;
...
window.openSchedulingBooking = openSchedulingBooking;
