...
/**
 * Removes Google Calendar events left behind when Scheduling bookings were cancelled.
 * Safe to re-run; only targets events linked to cancelled/completed bookings.
 *
 * Usage: node scripts/purge-cancelled-scheduling-calendar-events.js
 */
...
async function main() {
    ...
    try {
        ...
        for (const row of rows) {
            ...
            await pool.execute(
                'UPDATE scheduling_bookings SET google_calendar_event_id = NULL WHERE id = ?',
                [row.id]
            );
        }
