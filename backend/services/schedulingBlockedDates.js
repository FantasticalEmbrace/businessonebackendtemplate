...
async function tableExists(pool) {
    const [rows] = await pool.query(
        `SELECT COUNT(*) AS c FROM INFORMATION_SCHEMA.TABLES
         WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'scheduling_blocked_dates'`
    );
...
async function addBlockedDate(pool, dateYmd, reason = null, adminId = null) {
    ...
    try {
        await pool.execute(
            `INSERT INTO scheduling_blocked_dates (block_date, reason, created_by_admin_id)
             VALUES (?, ?, ?)`,
...
async function removeBlockedDate(pool, dateYmd) {
    ...
    const [result] = await pool.execute('DELETE FROM scheduling_blocked_dates WHERE block_date = ?', [ymd]);
...
async function ensureSchedulingBlockedDatesTable(pool) {
    try {
        if (await tableExists(pool)) return;
        ...
        await pool.query(`
            CREATE TABLE scheduling_blocked_dates (
                ...
                UNIQUE KEY uq_scheduling_blocked_date (block_date),
                INDEX idx_block_date (block_date)
            )
        `);
        logger.info('Database: created scheduling_blocked_dates table');
    } catch (e) {
        logger.warn(`Database: could not create scheduling_blocked_dates — ${logger.formatMysqlError(e)}`);
    }
}
...
module.exports = {
    ...
    addBlockedDate,
    removeBlockedDate,
    ensureSchedulingBlockedDatesTable,
};
